...
# ----------------- HALAMAN 2: REKOMENDASI (LAMA) -----------------
    ...
    if not available_nutrients:
        ...
        if st.button("Hitung Rekomendasi"):
            ...
            if recommendations:
                rec_df = pd.DataFrame(recommendations).sort_values(by="Skor", ascending=False).head(100)
...
# ----------------- HALAMAN 8: PANEL ADMIN -----------------
    ...
    if not st.session_state.logged_in:
        ...
        st.subheader("Edit Aturan Dual-Diagnosis Baru")
        ...
        st.subheader("Edit Sensitivitas Prediksi Penyakit")
